...
@click.option(
    "--unified-package", default=None, type=str, help="URL to unified package (for offline installer triggers)"
)
@click.option(
    "--nonroot-offline-install", default=None, type=click.BOOL, help="Install Scylla without required root privilege"
)
@click.option("--billing-project", default=None, type=str, help="Billing project for the test run")
@click.option(
    "--collect-report", is_flag=True, default=False, help="Wait for triggered jobs and collect results report"
)
...
def trigger_matrix_cmd(  # noqa: PLR0913
    ...
    if not scylla_version:
        ...
        if not has_image and not unified_package:
            click.echo(
    ...
    if not version_from_image and scylla_version:
        try:
    ...
    # Image params (scylla_ami_id, gce_image_db, etc.) are only used for version
    # resolution above — not passed to triggered jobs. Downstream jobs resolve
    # their own images from scylla_version.
    overrides = {}
    if stress_duration:
    ...
    if nonroot_offline_install is not None:
        overrides["nonroot_offline_install"] = str(nonroot_offline_install).lower()
    if billing_project:
        overrides["billing_project"] = billing_project

    if collect_report:
        # Plan-only mode: output JSON trigger plan for Groovy to execute with wait+report
        try:
        ...
        click.echo("TRIGGER_PLAN_JSON=" + json.dumps(plan))
        return

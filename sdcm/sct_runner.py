...
from sdcm.utils.aws_utils import ec2_instance_wait_public_ip, ec2_ami_get_root_device_name, tags_as_ec2_tags
...
class AwsSctRunner(SctRunner):
    ...
    def _create_instance(self,
        ...
        subnets = [aws_region.sct_subnet(region_az=region_az, subnet_index=f"{i}") for i in range(
            network_interfaces_count(self.params))]
        assert all(subnets), f"No SCT subnet found in the source region. " \
        ...
        if len(instance.network_interfaces) == 2:

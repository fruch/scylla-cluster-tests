...
def resolve_job_path(job_name: str, job_folder: str) -> str:
    ...
    if not job_folder:
        raise TriggerMatrixError(
            f"Job '{job_name}' uses a relative path but no job_folder could be determined. "
            "Provide --job-folder or --scylla-version, or use absolute paths in the matrix YAML."
        )
...
def plan_trigger_matrix(
    ...
    **overrides,
    ...
    """Generate a trigger plan without executing — returns a list of job specs for Groovy execution.

    Each spec contains 'job_path' and 'parameters' ready for Jenkins `build job:` step.
    """
    ...
    logger.info(
        "Plan: %s | Version: %s | Folder: %s | Jobs: %d/%d",
        matrix_file,
        scylla_version,
        resolved_folder,
        len(filtered),
    ...
    for job in filtered:
        ...
        plan.append({"job_path": full_path, "parameters": params})

...
class TestBuildJobParameters:
    ...
    def test_version_omitted_when_empty(self):
        ...
        assert "scylla_version" not in params

    def test_unified_package_passthrough(self):
        job = JobConfig(job_name="test", backend="aws", region="us-east-1")
        overrides = {"unified_package": "https://example.com/scylla.tar.gz", "nonroot_offline_install": "true"}
        params = build_job_parameters(job, {}, "", overrides)
        assert params["unified_package"] == "https://example.com/scylla.tar.gz"
        assert params["nonroot_offline_install"] == "true"
        assert "scylla_version" not in params
...
class TestResolveJobPath:
    ...
    def test_relative_path_no_folder_raises(self):
        with pytest.raises(TriggerMatrixError, match="relative path"):
            resolve_job_path("tier1/longevity-test", "")

    def test_absolute_path_no_folder_ok(self):
        assert resolve_job_path("/enterprise/perf/test", "") == "enterprise/perf/test"
...
# ---------------------------------------------------------------------------
# Phase 2: plan_trigger_matrix tests
# ---------------------------------------------------------------------------


class TestPlanTriggerMatrix:
    def test_plan_returns_job_specs(self, sample_matrix_yaml):
        plan = plan_trigger_matrix(
            matrix_file=str(sample_matrix_yaml),
            scylla_version="2025.4",
            labels_selector="weekly",
        )
        assert len(plan) == 2
        for spec in plan:
            assert "job_path" in spec
            assert "parameters" in spec
            assert spec["parameters"]["scylla_version"] == "2025.4"

    def test_plan_with_overrides(self, sample_matrix_yaml):
        plan = plan_trigger_matrix(
            matrix_file=str(sample_matrix_yaml),
            scylla_version="2025.4",
            labels_selector="weekly",
            unified_package="https://example.com/scylla.tar.gz",
            nonroot_offline_install="true",
        )
        for spec in plan:
            assert spec["parameters"]["unified_package"] == "https://example.com/scylla.tar.gz"
            assert spec["parameters"]["nonroot_offline_install"] == "true"

    def test_plan_empty_version_with_absolute_paths(self, tmp_path):
        data = {
            "defaults": {},
            "jobs": [
                {
                    "job_name": "/enterprise/perf/test-arm",
                    "backend": "aws",
                    "region": "us-east-1",
                },
            ],
        ...
        assert len(plan) == 1
        assert plan[0]["job_path"] == "enterprise/perf/test-arm"
        assert "scylla_version" not in plan[0]["parameters"]
        assert plan[0]["parameters"]["unified_package"] == "https://example.com/scylla.tar.gz"
